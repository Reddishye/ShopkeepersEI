...
public class ColorTranslator {
    ...
    private static final Pattern REMOVE_UNUSED_DECIMALS_PATTERN = Pattern.compile("%!REMOVE_UNUSED_DECIMALS:(\\d+\\.?\\d*)!%");
    private static final Pattern REMOVE_ALL_DECIMALS_PATTERN = Pattern.compile("%!REMOVE_ALL_DECIMALS:(\\d+\\.?\\d*)!%");
    ...
    private static final String[] COLOR_REPLACEMENTS = {
            "<black>", "<dark_blue>", "<dark_green>", "<dark_aqua>", "<dark_red>", "<dark_purple>", "<gold>", "<grey>",
    ...
    public static String replaceStandardPlaceholders(String message) {
        message = REMOVE_UNUSED_DECIMALS_PATTERN.matcher(message).replaceAll(mr -> mr.group(1).replaceAll("\\.0+$", ""));
        message = REMOVE_ALL_DECIMALS_PATTERN.matcher(message).replaceAll(mr -> mr.group(1).split("\\.")[0]);
        return message;
    }
